...
class SignedLoopBoundRule final : public Rule {
public:
    const char* id()    const noexcept override { return "perf-lens.ub.signed-loop-bound"; }
    const char* title() const noexcept override { return "signed/unsigned mismatch in loop condition"; }
