...
class UnorderedMapNoReserveRule final : public Rule {
public:
    const char* id()    const noexcept override { return "perf-lens.hotpath.unordered-map-no-reserve"; }
    const char* title() const noexcept override { return "unordered_map/set insertions without reserve"; }
