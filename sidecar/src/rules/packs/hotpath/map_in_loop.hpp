...
class MapInLoopRule final : public Rule {
public:
    const char* id()    const noexcept override { return "perf-lens.hotpath.map-in-loop"; }
    const char* title() const noexcept override { return "std::map/set lookup inside loop"; }
